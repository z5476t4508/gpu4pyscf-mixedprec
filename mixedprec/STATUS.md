...
**许可边界, 先说清楚**: Direwolf 是 **AGPL-3.0-or-later**, g16 是商业闭源。
两者的**代码都不能抄进 gpu4pyscf** (Apache-2.0) —— AGPL 有传染性, g16 是版权
问题。要借的都是公开方法 (COSX 是 Neese 2009, 两阶段网格是通行做法), **看它
怎么做、然后独立实现**, 这条路干净。g16 按用户指示只读代码不比性能。

### 两阶段 XC 网格: 实测 1.16x, 且只对单点有用

来源: `src/xc/grid_dynamic.f90` + `scf/scf_fock.f90:47`。先用粗网格跑 SCF,
密度稳下来后原地升到生产网格。三个设计细节:

- **触发是密度变化不是能量**: `Prms < xcgrid_switch_prms` (RMS 密度变化);
- **`Prms_round8(Prms)`** —— 判据取整到 8 位, 让开关决策在不同线程数下
  **可复现**。我的 conv_tol 调度没做这件事;
- **有 warm start 时自动关掉** (`engine.f90:453`)。

网格成本对等级 (r2SCAN/Tamoxifen/def2-SVP, 15 轮):

| level | 格点数 | SCF | XC | XC/轮 | E vs level 3 |
|---|---|---|---|---|---|
| **3** (默认) | 679680 | 6.64s | 4.84s | 0.323s | — |
| 2 | 469504 | 4.71s | 3.21s | 0.214s | -1.35e-03 |
| 1 | 216832 | 3.18s | 1.71s | 0.114s | +6.99e-03 |
| 0 | 48384 | 2.06s | 0.60s | 0.040s | +6.28e-01 |

两个原型, **差别只在 DIIS 是否保留**:

| 方案 | 耗时 | 总轮数 | 加速 | dE |
|---|---|---|---|---|
| 单阶段 level 3 | 6.61s | 15 | — | — |
| 粗版**重启** (L1 收到 1e-6, 拿密度重启 L3) | 6.47s | **19** | **1.02x** | 2.3e-12 |
| **原地切换** (rms<1e-3 换网格, DIIS 不动) | 5.71s | **15** | **1.16x** | 3.6e-12 |

**重启基本没收益**: 便宜的粗轮被多出的 4 轮昂贵迭代吃掉 (15→19)。这正是
「便宜的内层步会换来更多外层迭代」那条规则 —— 我预测到了才去数轮数。
**Direwolf 之所以原地切换而不是重启, 这就是原因。** 原地方案扫过粗网格等级
(L1/L2) × 触发阈值 (1e-2/-3/-4), 最好 1.16x, 轮数不变, 能量一致到 3.6e-12,
**零精度代价**。

**我的估计又乐观了 (估 1.53x, 实测 1.16x)。** 错的不是每轮成本, 是**粗网格能
覆盖的轮数**: 密度 rms 到第 5 轮就掉到 1e-3 以下, 只有约 5/15 轮能走粗网格,
不是我以为的 10/15。教训同 ip2: 省多少时间也得测。

**关键限制: 这个杠杆对几何优化几乎无用。** 优化第 2 步之后都是 warm start,
密度一开始就收敛, 粗网格阶段只白赚一次网格重建 —— Direwolf 正因此在
...
文献里 RIJCOSX 那个"快 4-10 倍"是拿 COSX 比**精确 K**。而 **gpu4pyscf 的 DF
路径已经在用 RI-K**(密度拟合的 K)。所以真正要比的是 **COSX vs RI-K**, margin
窄得多。而且 RI-K 在 GPU 上是稠密 GEMM —— 正是 GPU 最擅长的; COSX 是格点活儿。
**别把文献数字直接搬过来。**

#### 1. K 占多少 (实测, B3LYP/def2-SVP/Tamoxifen 537 AO, 混合通道)

    get_jk  J only      3.15 ms/次
    get_jk  K only    245.66 ms/次      <- K 比 J 贵 78 倍
    get_jk  J+K       249.03 ms/次

SCF 6.27s / 14 轮 → **K 一共 3.44s = SCF 的 55%**。

**天花板 (推算)**, 把"COSX 让 K 快 N 倍"代进去:
...
| COSX 3x | 3.98s | 1.58x |
| COSX 5x | 3.52s | 1.78x |
| **K 完全免费** (不可能) | 2.83s | **2.21x** |

**2.21x 是天花板**, 剩下 45% 是 XC 网格和其它。

#### 2. COSX 的**下界**成本 (实测)

COSX 里我能直接测的两块: 格点上求基函数值 (`eval_ao`) + 两个大 GEMM
(`X·D` 和 `Aᵀ·X`, 各 ngrid×nao×nao):

| COSX 格点 | eval_ao | 2×GEMM fp64 | 2×GEMM fp32 | 下界 fp64 | 下界 fp32 |
|---|---|---|---|---|---|
| 50k | 20.3ms | 38.4ms | 1.2ms | **58.7ms** | **21.5ms** |
| 100k | 39.1ms | 76.8ms | 2.4ms | **115.9ms** | **41.5ms** |
| 200k | 77.5ms | 152.4ms | 4.5ms | **229.9ms** | **82.0ms** |

(20k 那一档的 eval_ao 99.86ms 是首次调用的预热假象, 后三档线性缩放才可信。)
典型 COSX 格点约 100k, 比 DFT 的 679680 小得多。

#### 3. 差的那一步 —— 这是决定性的

**上表是下界, 不是估计: 它没包含 A 矩阵** (每个格点上的半数值库仑势积分),
而那才是 COSX 真正的大头。所以现在能说的只有:

> 100k 格点下, COSX 的"便宜部分"花掉 42-116 ms, **留给 A 矩阵的预算是
> 130-200 ms** (RI-K 是 245.9 ms)。A 能不能塞进这个预算, 就是 COSX 在 GPU 上
> 成不成的全部问题。

**下一步该量的**: A 矩阵和"格点上的静电势积分"是同一类积分, 如果 gpu4pyscf 里
有 `int1e_grids` 那类入口, **可以直接测出 A 的量级, 不用实现 COSX**。半小时的事,
比写实现便宜三个数量级。**没量出这个之前不要动手写 COSX。**

#### 4. 两个附带结论

- **COSX 天然该是 fp32 的**: 那两个 GEMM 在 fp32 下快 **32 倍**
  (76.8ms → 2.4ms)。而按本项目自己的规律, **SCF 里的 K 属于"已接受"那一类**
  (误差经过 SCF 自身收敛被压掉), 和落在二阶导上的情形完全不同。
- **只对杂化泛函有效**。纯泛函 (PBE/r2SCAN) 没有精确交换, K 的成本为零,
  COSX 收益也为零。而用户的 r2SCAN 正是纯泛函。

### 其它值得看的 (未展开)

- **统一内存预算**: `mem_cap_gb` 是全代码库唯一的预算旋钮, 所有
  STORE-vs-DIRECT/缓存决策都查它 (替代三个各自为政的环境变量)。gpu4pyscf 里
  同类决策散在各处 (`word_avail = mem_avail // 8` 等)。
- **`estimate_only` 模式**: 不算, 只报这个体系/基组要多少内存。批量场景有用。
- **位可复现的并行归约** (固定顺序 OpenMP 合并), 和 `Prms_round8` 同一思路:
  让结果不依赖线程数。
- **`docs/VALIDATION.md` 的写法**: 每条结论都是**对着具名外部代码**的数字
  (Psi4/ORCA/pyscf/Gaussian), 且**明确记录哪些不能交叉验证以及为什么**
  (开壳层 d 区原子两边会收敛到不同的合法态 —— 用闭壳层分子)。这正是我哨兵
  缺的那块。

### g16 已读到的 (只作参照)

`real*4` 全库只出现在 3 个文件 (`l1111.F`/`l923.F`/`newzmat.F`), **CPHF 和
Hessian 链 (`cphfutil.F`/`l1002.F`/`l1110.F`) 里一次都没有** —— 独立印证了
「误差落在二阶导上就不能用 fp32」这条我花两天测出来的规律。

### 编译状态 (2026-09-13 修正): **编译成功**, 二进制可跑, 旧结论作废

~~09-09 的「仍未编译成功」已过时~~: `Direwolf/Direwolf` 二进制 09-09 19:49 落地
(V0.99, build da057c5f46df), 冒烟通过 (water LDA/6-31g 收敛,
E = -76.013445954 Eh)。OpenBLAS 实测按 `TARGET=HASWELL` 编成 (AVX2 路径,
Arrow Lake 对口, config.h 证实) —— CPU 侧对比数字有效, 报告须注明此 TARGET。
两个坑 (CMAKE_POLICY_VERSION_MINIMUM / TARGET=HASWELL) 都按上文的解法落了地。

### 编译历史 (2026-09-09, 归档): 两个拦路的坑和解法
